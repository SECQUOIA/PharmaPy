...
def test_positive_reflux_below_minimum_uses_current_minimum(monkeypatch):
    DistillationColumn = _import_distillation_column(monkeypatch)
    ...
    monkeypatch.setattr(column, "calc_min_reflux", fake_calc_min_reflux)
    monkeypatch.setattr(column, "calc_num_min", fake_calc_num_min)

    result = column.calculate_heuristics()

    assert result["min_reflux"] == pytest.approx(min_reflux)
    assert result["reflux"] == pytest.approx(1.5 * min_reflux)


def test_calc_min_reflux_includes_feed_quality_in_underwood_target(monkeypatch):
    DistillationColumn = _import_distillation_column(monkeypatch)
    column = DistillationColumn(
        pres=101325.0,  # [Pa]
    ...
    min_reflux = column.calc_min_reflux(
        x_dist=x_dist,
        x_bot=x_bottom,
