...
class TestFlowsheets(unittest.TestCase):
    ...
    def test_B_BC_FILT(self):
        ...
        kinetics = RxnKinetics(
            path=path_phys, rxn_list=rxns, k_params=k_vals, ea_params=ea_vals
        )
        temp_init = 313.0  # Temperature in Kelvin
        mole_conc_init = np.array(
            [0.33, 0.33, 0, 0, 0]
        )  # Equimolar concentration of A, B (mol/L)
        liquid_init = LiquidPhase(
            path_phys,
            temp=temp_init,
            mole_conc=mole_conc_init,
            vol=0.06,
            name_solv="solvent",
        )
        temp_R01 = 313.15
        ...
        x_gr = np.geomspace(
            1, 1500, num=35
        )  # discretization point for crystal size distribution
        distrib_init = np.zeros_like(x_gr)
        massfrac_solid = [0, 0, 1, 0, 0]  # solid phase composed only by API (C)
        solid_cry = SolidPhase(
            path_phys, x_distrib=x_gr, distrib=distrib_init, mass_frac=massfrac_solid
        )
        temp_program = np.array(
            [
                [313.15, 308],
                [308, 295],
                [295, 278.15],
        ...
        flst.CR01.Kinetics = CrystKinetics(
            solub_cts, nucl_prim=prim, nucl_sec=sec, growth=growth, dissolution=dissol
        )
        ...
        sundials = {
            "maxh": 60
        }  # Maximum step size we take in time is 1 minute [Based on experience for crystallization]0
        runargs_CR01 = {"runtime": time_CR01, "sundials_opts": sundials}  # Runtime
        runargs_F01 = {
            "runtime": None,
            "deltaP": deltaP,
        }  # Note that deltaP is passed here
        run_kwargs = {"R01": runargs_R01, "CR01": runargs_CR01, "F01": runargs_F01}
    ...
    def test_SB_BC_FILT(self):
        ...
        )  # Only solvent and species B initially.
        vol_liq = 0.03
        liquid_init = LiquidPhase(
            path_phys,
            temp=temp_init,
        ...
        )  # Inputs in mol/L. (Solvent conc calculated internally)
        time_R01 = 7200.0  # Batch time in seconds
        flowrate_in = vol_liq / time_R01  # Flowrate in (m3/s)
        flow_inlet = LiquidStream(
            path_phys,
            temp=temp_inlet,
            mole_conc=conc_inlet,
            vol_flow=flowrate_in,
            name_solv="solvent",
        )
        temp_R01 = 313.15
    ...
    def test_PFR_HOLD_BC_FILT(self):
        ...
        w_init = np.array(
            [0, 0, 0, 0, 1]
        )  # mass fraction # Only solvent is present at the start
        liquid_init = LiquidPhase(
            path_phys, temp=313.15, mass_frac=w_init, vol=vol_liq
        )  # This is for the initial holdup existing in PFR
        temp_R01 = 313.15
        cw = CoolingWater(mass_flow=0.1, temp_in=temp_R01)
        c_in = np.array([0.33, 0.33, 0, 0, 0])
        temp_in = 40 + 273.15  # K
        liquid_in = LiquidStream(
            path_phys, temp_in, mole_conc=c_in, vol_flow=vol_flow, name_solv="solvent"
        )
    ...
    def test_PFR_MSMPR_HOLD_FILT(self):
        ...
        liquid_init = LiquidPhase(
            path_phys, temp=298, mass_frac=w_init, vol=vol_liq
        )  # This is for the initial holdup existing in PFR
        ...
        runargs_hold = {
            "runtime": time_CR01,
            "time_grid": np.arange(10800, 3600 * 10, 100),
        }
