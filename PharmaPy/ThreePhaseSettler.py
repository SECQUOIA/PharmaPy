...
class ThreePhaseSettler:
    def __init__(self, d_impeller, d_vessel, target_compound, agit_rate=1000 / 60):
    ...
    def design_space_check(
        ...
        perc_vol_basis = np.percentile(
            distrib * x_distrib**3 * 1e-9, 50, method="closest_observation"
        )  # distrib**3 to get volume basis, 1e-9 to convert to m3 and reduce number magnitude
        ...
        # 1st criterion
        vt_top = (
            g * d_particle**2 * (density_particle - density_top) / (18 * viscosity_top)
        )
        ...
        Ek = (
            0.5
            * (density_particle * np.pi * d_particle**3 / 6)
            * (d_impeller / 2 * (agit_rate / 2 * np.pi))
        )
        crit2 = Bo > 1 or Ek > Ea
        ...
        N_js = (
            ...
            * (viscosity_bot * 1000 / (density_bot / 1000)) ** (-1 / 9)
            / 60
        )
    ...
    def retrieve_results(self, time, states):
        ...
        solid_mass_flow = (
            solid_conc
            * self.Inlet_bot_phase.vol_flow
            * self.Inlet_bot_phase.getDensity()
        )

        OutletLiq = LiquidStream(
            path,
            mass_frac=self.Inlet_bot_phase.mass_frac,
            vol_flow=self.Inlet_bot_phase.vol_flow,
        )
        OutletSolid = SolidStream(
            path,
            mass_flow=solid_mass_flow,
            mass_frac=self.Inlet.Solid_1.mass_frac,
            temp=self.Inlet.Solid_1.temp,
            distrib=self.Inlet.Solid_1.distrib,
            x_distrib=self.Inlet.Solid_1.x_distrib,
        )
