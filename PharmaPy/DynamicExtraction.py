...
class DynamicExtractor:
    ...
    def get_stage_dimensions(self, di_states, rhos):
        vol = (
            di_states["holdup_heavy"][0] / rhos[0]
    ...
    def get_inputs(self, time):
        inputs = {
            key: get_inputs_new(time, inlet, self.states_in_dict)
            for key, inlet in self.Inlet.items()
        }
    ...
    def get_augmented_arrays(self, di_states, inputs):  # bottom_flows):
        ...
        temp_augm = np.hstack((temp_in["feed"], di_states["temp"], temp_in["solvent"]))
    ...
    def unit_model(self, time, states, sdot=None):
        ...
        # ---------- Unpack variables
        di_states = unpack_discretized(states, self.dim_states, self.name_states)

        if sdot is None:
            di_sdot = sdot
        else:
            di_sdot = unpack_discretized(sdot, self.dim_states, self.name_states)
        ...
        # ---------- Balances
        material = self.material_balances(
            time,
    ...
    def material_balances(
        ...
        # ---------- Algebraic block
        equilibrium_alg = m_ij * (x_augm[1:] - x_augm[:-1] * (1 - self.eff)) - y_i

        # ---------- Modify outputs for stage two on
        if self.num_stages > 1:
            deriv_term = (
                holdup_heavy[0] * m_ij * (1 - self.eff) / div[1:] * dxij_dt[:-1]
            )
    ...
    def energy_balances(
        ...
        h_light = self.Liquid_1.getEnthalpy(
            mole_frac=x_augm, temp=temp_augm[:-1], basis="mole"
        )

        h_heavy = self.Liquid_1.getEnthalpy(
            mole_frac=y_augm, temp=temp_augm[1:], basis="mole"
        )

        duint_dt = (
            heavy_flows[1:] * h_heavy[1:]
        ...
        temp_eqns = holdup_heavy * h_heavy[:-1] + holdup_light * h_light[1:] - u_int
    ...
    def initialize_model(self):
        ...
        rhos_streams = {
            key: obj.getDensity(basis="mole") for key, obj in self.Inlet.items()
        }
        ...
        rhos_holdups = [
            self.Liquid_1.getDensity(
                mole_frac=di_init[key], basis="mole", temp=di_init["temp"]
            )
        ...
        h_light = self.Liquid_1.getEnthalpy(
            mole_frac=di_init["x_i"], temp=di_init["temp"], basis="mole"
        )

        h_heavy = self.Liquid_1.getEnthalpy(
            mole_frac=di_init["y_i"], temp=di_init["temp"], basis="mole"
        )
    ...
    def solve_unit(self, runtime, sundials_opts=None, verbose=True):
        ...
        problem = Implicit_Problem(self.unit_model, y0=init_states, yd0=init_deriv)
    ...
    def retrieve_results(self, time, states):
        ...
        frac_keys = ("x_i", "y_i")
        frac_last = {
            key: np.column_stack(list(di_last[key].values())) for key in frac_keys
        }
        ...
        di_last["mole_flow"] = np.repeat(
            kws[self.default_output]["mole_flow"], len(time)
        )
    ...
    def plot_profiles(self, times=None, stages=None, pick_comp=None, **fig_kwargs):
