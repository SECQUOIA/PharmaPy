...
class Drying:
    ...
    def nomenclature(self):
        ...
        self.name_states = ["saturation", "y_gas", "x_liq", "temp_gas", "temp_cond"]

        index_z = list(range(self.num_nodes))

        name_liq = [
            name for name in self.name_species if name not in self.supercrit_names
        ]
        self.states_di = {
            ...
            "temp_gas": {"dim": 1, "units": "K", "type": "diff"},  # 'index': index_z,
            "temp_cond": {"dim": 1, "units": "K", "type": "diff"},  # 'index': index_z,
        }
    ...
    def unit_model(self, time, states, sw=None):
        ...
        x_liq = states_reord[:, 1 + num_comp : 1 + num_comp + self.num_volatiles]
        x_liq[:, -2] = 0
        temp_gas = states_reord[:, -2]
        temp_sol = states_reord[:, -1]

        # ---------- Darcy's equation
        visc_gas = self.Vapor_1.getViscosity(temp=temp_gas, mass_frac=y_gas)
        ...
        energy_eqns = self.energy_balance(
            time,
            temp_gas,
            temp_sol,
            satur,
    ...
    def energy_balance(
        ...
        # ----- Gas phase equations
        cpg_mix = self.Vapor_1.getCp(temp=temp_gas, mass_frac=y_gas, basis="mass")
        cvg_mix = cpg_mix - gas_ct / mw_avg_gas * 1000  # J/kg K

        epsilon_gas = self.porosity * (1 - satur)
        denom_gas = cvg_mix * epsilon_gas * rho_gas

        latent_heat = self.Vapor_1.getHeatVaporization(temp_sol, basis="mass")

        xliq_extended = np.column_stack(
            (x_liq, np.zeros((x_liq.shape[0], len(self.idx_supercrit))))
        )

        cpl_mix = self.Liquid_1.getCp(
            temp=temp_sol, mass_frac=xliq_extended, basis="mass"
        )
        temp_wb = 22 + 273
        ...
        fluxes_Tg = high_resolution_fvm(temp_gas, boundary_cond=temp_gas_inputs)
        ...
        denom_cond = (
            self.rho_sol * (1 - self.porosity) * self.cp_sol
    ...
    def solve_unit(
        ...
        if x_liq_init.ndim == 1:
            ...
            states_stacked = np.column_stack(
                (satur_init, y_gas_init, x_liq_init, temp_gas_init, temp_cond_init)
            )
        ...
        rho_liq = self.Liquid_1.getDensity(
            temp=temp_cond_init, mass_frac=xliq_init, basis="mass"
        )
        surf_tens = self.Liquid_1.getSurfTension(
            temp=temp_cond_init, mass_frac=xliq_init
        )
        ...
        deltaP_media = (
            deltaP
            * self.resist_medium
            / (alpha * rho_sol * (1 - porosity) * self.cake_height + self.resist_medium)
        )
        deltaP -= deltaP_media
        self.deltaP = deltaP
        p_top = p_atm + deltaP
        ...
        self.pres_gas = np.linspace(p_top, p_top - deltaP, num=self.num_nodes)
        ...
        if self.state_event_list is None:
            ...
            def model(t, y):
                return self.unit_model(t, y)  # , None)

            problem = Explicit_Problem(self.unit_model, y0=states_prev.ravel(), t0=0)
        else:
            switches = [True] * len(self.state_event_list)
            problem = Explicit_Problem(
                self.unit_model, y0=states_prev.ravel(), t0=0, sw0=switches
            )
    ...
    def plot_profiles(self, times=None, z_pos=None, pick_comp=None, **fig_kw):
        ...
        if z_pos is not None:  # TODO: this needs to be implemented
            ...
            axes[0].text(
                1,
                1.04,
                "z_pos = %.3e m" % self.z_centers[idx_node],
                ha="right",
    ...
    def plot_rates(self, z_pos=0, fig_size=None):
        ...
        axis.text(
            1,
            1.04,
            "$z = %.2f$ m" % self.z_centers[z_idx],
            ha="right",
            transform=axis.transAxes,
        )

        axis.legend(
            [self.Liquid_1.name_species[ind] for ind in self.idx_volatiles], loc="best"
        )
