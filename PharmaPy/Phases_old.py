...
class LiquidPhase(ThermoPhysicalManager):
    ...
    def to_stream(self):
        ...
        return LiquidStream(
            path_thermo=self.path_data,
            temp=self.temp,
            pres=self.pres,
            mass_flow=self.mass,
            mass_frac=self.mass_frac.copy(),
            name_solv=None if self.ind_solv is None else self.name_species[self.ind_solv],
            check_input=False,
            verbose=False,
    )
...
class SolidPhase(ThermoPhysicalManager):
    ...
    @property
    def vol(self):
        return self.mass/self.getDensity(mass_frac = self.mass_frac,basis='mass')
    @property
    def mass_j(self):
        return self.mass*self.mass_frac
    ...
    def updatePhase(self, x_distrib=None, distrib=None, mass=None,
                    moments=None, temp=None,mass_j=None,vol=None):
        if temp is not None:
            self.temp= temp
        if mass_j is not None:
            self.mass_j = mass_j
        ...
        if moments is not None:
            self.moments = moments
        if vol is not None:
            self.mass = self.getDensity(mass_frac=self.mass_frac)*vol
    ...
    #TODO fix to_stream, distrib and x_distrib don't carry over but are deprecated so may not be an issue