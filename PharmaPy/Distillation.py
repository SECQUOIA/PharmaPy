...
class _BaseDistillation:
    ...
    def nomenclature(self):  # TODO: We might need to make this child-specific
        ...
        self.fstates_di = {
            "y_vap": {"dim": len(self.name_species), "index": self.name_species}
        }
    ...
    def get_alpha(self, pres, x_frac):
        ...
        k_vals = self.Inlet.getKeqVLE(
            temp=temp_bubble, pres=pres, x_liq=x_frac, gamma_model=self.gamma_model
        )
    ...
    def global_material_bce(self, z_feed=None):
        ...
        temp_bubble_feed = self.Inlet.getBubblePoint(pres=self.pres, mole_frac=z_feed)

        k_feed = self.Inlet.getKeqVLE(temp=temp_bubble_feed, pres=self.pres)
        volatility_order = np.argsort(k_feed)[::-1]
        self.sorted_by_volatility = [self.name_species[ind] for ind in volatility_order]
        ...
        if hk_loc != lk_loc + 1:
            ...
            print(
                "Volatility order at T_bubble = %.1f K (%.0f Pa, high to low): "
                % (temp_bubble_feed, self.pres)
                + "-".join(self.sorted_by_volatility)
            )

        # ---------- Calculate Distillate and Bottom flow rates
        bot_flow = feed_flow * (
            z_feed[HK_index] * ((1 - self.frac_HK))
        ...
        x_bot[lighter_idx] = (
            feed_flow * z_feed[lighter_idx] * (1 - self.x_NLK) / bot_flow
        )
        x_bot[LK_index] = feed_flow * z_feed[LK_index] * (1 - self.frac_LK) / bot_flow
        x_bot[HK_index] = feed_flow * z_feed[HK_index] * (1 - self.frac_HK) / bot_flow
        x_bot[heavier_idx] = (
            feed_flow * z_feed[heavier_idx] * (1 - self.x_NHK) / bot_flow
        )
    ...
    def molokanov_equation(self, reflux, min_reflux, num_min):
        ...
        y_val = 1 - np.exp(
            (1 + 54.4 * x_val) / (11 + 117.2 * x_val) * (x_val - 1) / np.sqrt(x_val)
        )
    ...
    def calc_min_reflux(self, x_dist, x_bot, dist_flowrate, bot_flowrate, z_feed=None):
        if z_feed is None:
            z_feed = self.z_feed
        ...
        def f(phi):
            fun = (sum(alpha * z_feed / (alpha - phi + np.finfo(float).eps))) ** 2
    ...
    def calculate_heuristics(self, time=None):
        ...
        x_dist, x_bot, dist_flowrate, bot_flowrate = self.global_material_bce(z_feed)

        min_reflux = self.calc_min_reflux(
        ...
        mat_bce = {
            "x_dist": x_dist,
            "x_bottom": x_bot,
            "dist_flow": dist_flowrate,
            "bottom_flow": bot_flowrate,
        }
        ...
        if self.num_plates == 0 or self.num_plates is None:
            ...
            if self.num_plates < np.ceil(num_min):
                print(
                    "Warning: Specified number of plates (%i) is lower than "
        ...
        if self.num_feed is None:
            ...
            raise ValueError("Feed number is larger than total number of plates")
        else:
            num_feed = self.num_feed

        out = {
            "material_balances": mat_bce,
            "min_reflux": min_reflux,
            "num_min": num_min,
            "reflux": reflux,
...
class DistillationColumn(_BaseDistillation):
    ...
    def VLE(self, y_oneplate=None, temp=None, need_x_vap=True):
        ...
        temporary_vapor = VaporStream(
            path_thermo=self._Inlet.path_data,
            pres=self.pres,
            mole_flow=self.feed_flowrate,
            mole_frac=y_oneplate,
        )
        res = temporary_vapor.getDewPoint(
            pres=self.pres,
            mass_frac=None,
            mole_frac=y_oneplate,
            thermo_method=self.gamma_model,
            x_liq=need_x_vap,
        )
    ...
    def calc_plates(self, x_dist, x_bottom, dist_flow, bottom_flow, reflux, num_plates):
        ...
        if num_plates is None:
            ...
            # Rectifying section
            while (
                y_top_op[LK_index] / y_top_op[HK_index]
                < y_bot_op[LK_index] / y_bot_op[HK_index]
            ):
            ...
            while (
                np.array(x[-1][HK_index]) < 0.98 * x_bottom[HK_index]
                or np.array(x[-1][LK_index]) > 1.2 * x_bottom[LK_index]
            ):
...
class DynamicDistillation(_BaseDistillation):
    ...
    def unit_model(self, time, states, d_states, sw=None):
        ...
        di_d_states = unpack_discretized(d_states, self.len_out, self.name_states)
    ...
    def material_balances(self, time, temp, x_liq):
        ...
        (bot_flowrate, dist_flowrate, reflux, num_feed, M_const) = (
            self.bot_flowrate,
            self.dist_flowrate,
            self.reflux,
            self.num_feed,
    ...
    def solve_unit(
        ...
        if runtime is not None and time_grid is not None:
            raise RuntimeError(
                "Both 'runtime' and 'time_grid' were provided. "
        ...
        temp_init = self.Liquid_1.getBubblePoint(pres=self.pres, mole_frac=x_init)

        init_states = np.tile(np.hstack((temp_init, x_init)), (self.num_plates + 1, 1))

        init_derivative = self.material_balances(
            time=self.elapsed_time, x_liq=init_states[:, 1:], temp=init_states[:, 0]
        )
        ...
        time, states, d_states = solver.simulate(final_time, ncp_list=time_grid)
    ...
    def retrieve_results(self, time, states):
        ...
        dp = unpack_discretized(
            states, self.len_out, self.name_states, indexes=indexes, inputs=None
        )
        ...
        for ind, row in enumerate(dp["temp"]):
            x_liquid = np.column_stack([val[ind] for val in dp["x_liq"].values()])

            k_vals = self.Liquid_1.getKeqVLE(pres=self.pres, temp=row, x_liq=x_liquid)
        ...
        self.result = DynamicResult(
            di_states=self.states_di, di_fstates=self.fstates_di, **dp
        )

        outputs = retrieve_pde_result(self.result, x_name="plate", x=self.num_plates)
