...
def check_stoichiometry(stoich, mws):
    ...
    if not np.allclose(mass_bce, 0):
        print(
            "Warning! Material balance closure not attained with "
...
def get_sundials_callable(events, eval_sens, param_vals, unit_model, get_jac):
    ...
    if not flag_events and not eval_sens:
        ...
        def jac_fun(t, y):
            return get_jac(t, y, None, None, param_vals)
    ...
    # This is redundant but helps with readability
        ...
        def jac_fun(t, y, sw):
            return get_jac(t, y, sw, None, param_vals)
...
class _BaseReactor:
    ...
    def set_names(self):
        ...
        if self.isothermal:
            ...
            if reactor_type != "PlugFlowReactor":
                self.states_di["temp_ht"] = {"units": "K", "dim": 1, "type": "diff"}
    ...
    def unit_model(self, time, states, sw=None, params=None):
        ...
        di_states = complete_dict_states(
            time, di_states, ("vol", "temp", "temp_ht"), self.Liquid_1, self.controls
        )

        self.Liquid_1.temp = di_states["temp"]

        material_bces = self.material_balances(time, **di_states, inputs=u_values)

        if "temp" in self.states_uo:
            energy_bce = self.energy_balances(time, **di_states, inputs=u_values)
    ...
    def get_jacobians(self, time, states, sw, sens, params, wrt_states=True):
        ...
        if wrt_states:
            ...
            jac_theta_kin = self.Kinetics.derivatives(conc, temp, dstates=False)
    ...
    def paramest_wrapper(
        ...
        if self.return_sens:
            t_prof, states, sens = self.solve_unit(
                time_grid=t_vals, verbose=False, eval_sens=True, **run_args
            )
            ...
            t_prof, states = self.solve_unit(
                time_grid=t_vals, verbose=False, eval_sens=False, **run_args
            )
    ...
    def plot_sens(
        self, fig_size=None, mode="per_parameter", black_white=False, time_div=1
    ):
...
class BatchReactor(_BaseReactor):
    ...
    def material_balances(self, time, mole_conc, vol, temp, temp_ht, inputs):
        ...
        if self.Kinetics.keq_params is None:
            ...
            rate = self.Kinetics.get_rxn_rates(mole_conc, temp, delta_hrxn=deltah_rxn)
    ...
    def energy_balances(
        self, time, mole_conc, vol, temp, temp_ht, inputs, heat_prof=False
    ):
        ...
        deltah_rxn = self.Liquid_1.getHeatOfRxn(
            stoich, temp, self.mask_species, delta_href, tref_hrxn
        )  # J/mol

        rates = self.Kinetics.get_rxn_rates(
            mole_conc, temp, overall_rates=False, delta_hrxn=deltah_rxn
        )
    ...
    def solve_unit(
        ...
        problem = Explicit_Problem(
            call_fn, states_init, t0=self.elapsed_time, **kw_problem
        )

        if eval_sens:
            problem.jac = jac_fn

            def rhs_sens(t, y, sens, params):
                return self.get_jacobians(t, y, None, sens, params, wrt_states=False)
        ...
        if len(self.state_event_list) > 0:
            ...
            def new_handle(solver, info):
                return handle_events(
                    solver, info, self.state_event_list, any_event=True
                )
        ...
        self.derivatives = call_fn(
            self.elapsed_time, states_init, *list(kw_problem.values())
        )
    ...
    def retrieve_results(self, time, states):
        ...
        dp = complete_dict_states(
            time, dp, ("vol", "temp"), self.Liquid_1, self.controls
        )

        if "temp_ht" in self.name_states:
            heat_prof = self.energy_balances(**dp, inputs=None, heat_prof=True)
        else:
            heat_prof = self.energy_balances(
                temp_ht=None, **dp, inputs=None, heat_prof=True
            )
        ...
        self.Liquid_1.updatePhase(vol=self.Liquid_1.vol, mole_conc=concentr_final)
...
class CSTR(_BaseReactor):
    ...
    def material_balances(self, time, mole_conc, vol, temp, temp_ht, inputs):
        ...
        if self.Kinetics.keq_params is None:
            rate = self.Kinetics.get_rxn_rates(mole_conc[self.mask_species], temp)
        else:
            deltah_rxn = self.Liquid_1.getHeatOfRxn(temp, self.Kinetics.tref_hrxn)

            rate = self.Kinetics.get_rxn_rates(
                mole_conc[self.mask_species], temp, deltah_rxn
            )
    ...
    def energy_balances(
        ...
        # Enthalpy calculations
        _, cp_j = self.Liquid_1.getCpPure(temp)  # J/mol
        h_tempj = self.Liquid_1.getEnthalpy(
            temp, self.temp_ref, total_h=False, basis="mole"
        )
        ...
        deltah_rxn = self.Liquid_1.getHeatOfRxn(
            self.Kinetics.stoich_matrix, temp, self.mask_species, deltah_ref, tref_dh
        )  # J/mol

        rates = self.Kinetics.get_rxn_rates(
            mole_conc.T[self.mask_species].T,
            temp,
            overall_rates=False,
        ...
        # Inlet stream
        stream = self.Inlet
        h_inj = stream.getEnthalpy(
            inlet_temp, temp_ref=self.temp_ref, total_h=False, basis="mole"
        )
        ...
        if heat_prof:
            ...
            heat_profile = np.column_stack((source_term, -ht_term, flow_term))
            return heat_profile
        else:
            ht_term = self.heat_transfer(temp, temp_ht, vol)
    ...
    def solve_unit(
        ...
        if eval_sens:
            ...
            def fobj(time, states):
                return self.unit_model(time, states, merged_params)

            problem = Explicit_Problem(fobj, states_init, t0=self.elapsed_time)
    ...
    def retrieve_results(self, time, states):
        ...
        if "temp_ht" in self.name_states:
            heat_prof = self.energy_balances(**dp, inputs=inputs, heat_prof=True)
        else:
            heat_prof = self.energy_balances(
                temp_ht=None, **dp, inputs=inputs, heat_prof=True
            )
        ...
        if self.__class__.__name__ == "SemibatchReactor":
            self.Liquid_1.updatePhase(
                temp=dp["temp"][-1], mole_conc=dp["mole_conc"][-1], vol=dp["vol"][-1]
            )
            self.Outlet = self.Liquid_1
        else:
...
class PlugFlowReactor(_BaseReactor):
    ...
    def material_steady(self, conc, temp):
        if self.Kinetics.keq_params is None:
            ...
            rate = self.Kinetics.get_rxn_rates(conc, temp, delta_hrxn=deltah_rxn)
    ...
    def energy_steady(self, conc, temp):
        ...
        rates = self.Kinetics.get_rxn_rates(
            conc, temp, overall_rates=False, delta_hrxn=deltah_rxn
        )
    ...
    def material_balances(self, time, mole_conc, vol_diff, temp, flow_in, rate_j):
        ...
        dconc_dt = (
            -flow_in * (diff_conc.T / vol_diff).T + rates[1:]
        )  # TODO Is it correct to ignore rate at V = 0?

        return dconc_dt

    def energy_balances(
        self, time, mole_conc, vol_diff, temp, flow_in, rate_i, heat_profile=False
    ):
        ...
        if heat_profile:
            ...
            dtemp_dt = flow_term + (source_term[1:] - heat_transfer[1:]) / cp_vol[1:]
    ...
    def unit_model(self, time, states, sw=None, params=None, enrgy_bce=False):
        ...
        di_states = unpack_discretized(states, self.len_states, self.name_states)
        ...
        if self.Kinetics.keq_params is None:
            ...
            rates_i = self.Kinetics.get_rxn_rates(
                conc_partic, temp_all, delta_hrxn=deltah_rxn, overall_rates=False
            )

        rates_j = np.dot(rates_i, self.Kinetics.normalized_stoich.T)

        material_bces = self.material_balances(
            time, **di_states, vol_diff=vol_diff, flow_in=flow_in, rate_j=rates_j
        )
        if enrgy_bce:
            ht_inst = self.energy_balances(
                time,
                **di_states,
                vol_diff=vol_diff,
                flow_in=flow_in,
    ...
    def solve_unit(
        ...
        c_init = np.ones((self.num_discr, self.num_species)) * self.Liquid_1.mole_conc
        ...
        model, jac_fn, kw_model = get_sundials_callable(
            ...
            get_jac=self.get_jacobians,
        )

        problem = Explicit_Problem(model, states_init, t0=self.elapsed_time, **kw_model)
        ...
        self.derivatives = model(
            self.elapsed_time, states_init, *list(kw_model.values())
        )
    ...
    def retrieve_results(self, time, states):
        ...
        indexes = {
            key: self.states_di[key].get("index", None) for key in self.name_states
        }

        inputs = self.get_inputs(time)["Inlet"]

        dp = unpack_discretized(
            states, self.dim_states, self.name_states, indexes=indexes, inputs=inputs
        )
        ...
        outlet_states = retrieve_pde_result(
            self.result, x_name="vol", x=self.vol_discr[-1]
        )
    ...
    def plot_profiles(self, times=None, vol=None, pick_comp=None, **fig_kw):
        ...
        if times is not None:
            ...
            if vol is None:
                raise RuntimeError(
                    "Please provide a volume value using " "the argument 'vol'"
                )
    ...
    def animate_reactor(
        self, filename=None, step_data=2, fps=5, pick_idx=None, title=None
    ):
        ...
        ax_temp.set_ylim(temp.min() - temp_diff * 0.03, temp.max() + temp_diff * 0.03)
