...
class DeliquoringStep:
    def __init__(self, num_nodes, diam_unit=0.01, resist_medium=1e9):
        """
    ...
    def Phases(self, phases):
        ...
        self.states_di = {
            "saturation": {"dim": 1, "units": "", "type": "diff"},  # 'index': index_z,
            "mass_conc": {
                "index": self.name_species,
                "dim": len(self.name_species),
                "units": "kg/m**3",
                "type": "diff",
            },  # Order of the states matters
        }
    ...
    def material_balance(self, theta, sat_star, conc_star):
        ...
        lambd = 5
        sat_star = np.where(
            sat_star < 0, eps, sat_star
        )  # Changing the negative element for numerical issue
    ...
    def solve_unit(self, deltaP, runtime, p_atm=101325, verbose=True):
        ...
        conc_init = define_initial_state(
            state=conc_upstream,
            z_after=z_dim,
            z_before=self.CakePhase.z_external,
            indexed_state=True,
        )
        ...
        for i in range(len(self.Liquid_1.name_species)):
            self.conc_mean_init[:, i] = (
                trapezoidal_rule(z_dim, conc_init[:, i]) / self.cake_height
            )
    ...
    def retrieve_results(self, theta, states):
        ...
        dp_reduced = unpack_discretized(
            states, self.dim_states, self.name_states, indexes=indexes
        )
        ...
        self.mean_sat = (
            trapezoidal_rule(self.z_centers, s_red.T) * (1 - self.sat_inf)
            + self.sat_inf
        )
        ...
        for name in indexes["mass_conc"]:
            concPerVolElement[name] = (
                concPerSpecies[name] * conc_diff[:, ind] + self.conc_mean_init[:, ind]
            )
    ...
    def plot_profiles(
        ...
        if mean_sat:
            ...
            profiles = [
                self.concPerSpecies[ind][idx_time]
    ...
    def animate_unit(
        self, filename=None, title=None, pick_idx=None, step_data=2, fps=5
    ):
        ...
        time_tag = ax_anim.text(
            1,
            1.04,
            "$time = {:.2f}$ s".format(self.timeProf[0]),
...
class Filter:
    def __init__(self, station_diam, alpha=None, resist_medium=1e9, log_params=False):
        """
    ...
    def Phases(self, phases):
        ...
        if self.alpha is None:
            self.alpha = get_alpha(
                self.Solid_1, sphericity=1, porosity=epsilon, rho_sol=dens_sol
            )
    ...
    def unit_model(self, time, states, sw):
        if self.oper_mode == "Batch":
            mass_liquid = states
            material_balance = self.material_balance(time, mass_liquid, self.deltaP)
    ...
    def solve_unit(
        ...
        # Solve ODE
        problem = Explicit_Problem(self.unit_model, y0=mass_init, t0=0, sw0=[True])
    ...
    def paramest_wrapper(
        self, params, time_vals, modify_phase=None, modify_controls=None
    ):

        self.reset()

        deltaP = self.deltaP
        time, states = self.solve_unit(
            time_grid=time_vals, deltaP=deltaP, model_params=params, verbose=False
        )
    ...
    def plot_profiles(self, time_div=1, black_white=False, **fig_kwargs):
        ...
        # Liquid mass
        if black_white:
            ax[0].plot(time_plot, mass_filtr, time_plot, mass_up, "--", color="k")
            ax[1].plot(
                time_plot, self.cake_dry, time_plot, self.cake_wet, "--", color="k"
            )
        else:
            ax[0].plot(time_plot, mass_filtr, time_plot, mass_up, "--")
            ax[1].plot(
                time_plot,
                self.result.mass_cake_dry,
                time_plot,
                self.result.mass_cake_wet,
                "--",
            )
...
class DisplacementWashing:
    def __init__(
        self, solvent_idx, num_nodes, diam_unit=None, resist_medium=1e9, k_ads=0
    ):
        """
    ...
    def material_balance(self, z_pos, time, vel, diff, lambd):
        ...
        arg_one = 0.5 * np.sqrt(lambd * vel**2 * np.einsum("i,j->ij", time, 1 / diff))
        arg_two = (
            0.5
            * np.sqrt(1 / lambd)
            * np.einsum("i,j,k->ijk", z_pos, 1 / np.sqrt(time), 1 / np.sqrt(diff))
        )
    ...
    def material_bce(self, z_pos, wash_ratio, vel, l_total, diff, lambd):
        ...
        conc_star = 1 - 0.5 * (
            erfc(np.outer(arg_one, root)) + exp_term * erfc(np.outer(arg_two, root))
        )

        return conc_star

    def solve_unit(
        self, deltaP, wash_ratio=1, time_vals=None, dynamic=True, verbose=True
    ):
        ...
        if dynamic:
            conc_adim = self.material_balance(
                z_vals, time_vals, vel_liq, diff, lambd_ads
            )

            conc_star = conc_adim[:, -1]
        else:
            conc_adim = self.material_bce(
                z_vals, wash_ratio, vel_liq, cake_height, diff, lambd_ads
            )
    ...
    def retrieve_results(self, z_coord, time_coord, conc):
        ...
        dp = unpack_discretized(
            conc_T, self.dim_states, self.name_states, indexes=indexes
        )
    ...
    def plot_profiles(self, fig_size=None, z_val=None, time=None, pick_idx=None):
        """
...
if __name__ == "__main__":
    plt.style.use("../../../publications/2021_CACE/source/cace_palatino.mplstyle")
