    ...
    for two in second:
        for count, one in enumerate(first):
            if "distr" in one and "solid_conc" in two:
                ...
                break
            elif any(word in one for word in comp_names) and any(
                word in two for word in comp_names
            ):
                graph[two] = one
                ...
                types["distrib"] = (one, two)
                break
            elif any(one == word for word in amount_names) and any(
                two == word for word in amount_names
            ):
                graph[two] = one
...
def get_types(names):
    conc_type = [name for name in names if ("conc" in name) or ("frac" in name)][0]

    distrib_type = [name if "distr" in name else None for name in names][0]
...
class NameAnalyzer:
    ...
    def __convertComposition(self, prefix_up, prefix_down, composition, matter_object):
    ...
    def __convertFlow(
        self, prefix_up, prefix_down, flow, matter_object, composition, comp_name
    ):
        up, down = prefix_up, prefix_down
    ...
    def __convert_distrib(self, prefix_up, prefix_down, distrib, matter_object):
        up, down = prefix_up, prefix_down
        ...
        if "distrib" in up and "total" in down:
            ...
            out = matter_object.getSolidsConcentr(distrib=distrib, basis="mass")
...
if __name__ == "__main__":
    ...
    # With name analyzer
    analyzer = NameAnalyzer(names_up, names_down, num_species, num_distr)
