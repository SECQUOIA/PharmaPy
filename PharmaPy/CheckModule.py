...
def check_modeling_objects(uo, instance_name=None):
    ...
    cond_kin = (
        module_name in checks["has_kinetics"]["modules"]
        or class_name in checks["has_kinetics"]["classes"]
    ) and "Kinetics" not in modeling_objs

    cond_utility = (
        module_name in checks["has_utility"]["modules"]
        or class_name in checks["has_utility"]["classes"]
    ) and "Utility" not in modeling_objs
    ...
    if len(missing_obj) > 0:
        intro = (
            "The following PharmaPy modeling objects were "
            "not detected in %s:\n" % instance_descr
        )
        ...
        recommend = (
            "Please create the missing modeling objects listed above"
