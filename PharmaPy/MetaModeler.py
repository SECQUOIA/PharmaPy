...
class MetaModelingClass:
    ...
    def __write_method(self, open_object, method_name, arg_names, internals=None):
        ...
        if method_name == "unit_model" and self.name_states is not None:
            if self.model_type == "ODE" or self.model_type == "DAE":
                ...
                assign_snippet = [
                    ...
                    " " * 8
                    + "states_split = np.split(states_reord, self.acum_len, axis=1)\n\n",
                    " " * 8
                    + "dict_states = dict(zip(self.name_states, states_split))\n",
                    "\n",
                ]
            ...
            open_object.write(
                " " * 8 + "Fill material and energy balances with your model.'''\n\n"
            )
            ...
            if self.model_type == "ODE":
                concat_snippet = (
                    " " * 8 + "balances = np.concatenate(material, energy)\n"
                )
            else:
            ...
            open_object.write(
                " " * 8 + "# If the model has stages, these nums "
            ...
            open_object.write(" " * 8 + "self.acum_len = len_states.cumsum()[:-1]\n")

            open_object.write(" " * 8 + "self.len_states = len_states\n\n")

            open_object.write(" " * 8 + "self.num_states = len_states.sum()\n\n")

            if self.model_type == "ODE":
                init_states = [" " * 8 + "init_states = np.hstack(())\n\n"]
            else:
                init_states = [
                    " " * 8 + "init_states = np.hstack(())\n",
            ...
            if self.model_type == "ODE":
                assign_outputs = [
                    ...
                    " " * 8
                    + "model_outputs = dict(zip(self.name_states, outputs_split))\n",
                ]
                ...
                assign_outputs = [
                    " " * 8
                    + "model_outputs = reorder_pde_outputs(states, self.num_nodes, self.len_states)"
                ]
    ...
    def CreatePharmaPyTemplate(self):
        ...
        displayed_args_init = [
            " " * 8 + "self.{} = {}\n".format(arg, arg) for arg in args_init
        ]
...
if __name__ == "__main__":
    ...
    meta_object = MetaModelingClass(
        name_file, name_class, name_states=states, model_type="DAE"
    )
